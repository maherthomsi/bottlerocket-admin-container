...
update_fips_crypto_policies() {
  [[ ${FIPS_MODE_FLAG} -eq 1 ]] && update-crypto-policies --set FIPS 2>/dev/null
...
readonly host_journal_socket="/.bottlerocket/rootfs/run/systemd/journal/socket"
...
if [[ -S "${host_journal_socket}" ]]; then
  ...
  log "Journal linking to host journal enabled"
fi
